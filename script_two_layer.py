...
results_dir = os.path.join(home_dir, 'results', '03-04', 'two_layer')
if os.path.exists(results_dir) == False:
        os.makedirs(results_dir)

#results_name=None
results_name = 'set_'+str(init_set)+'_model_type_'+str(model_type)+'_noise_type_'+str(noise_type)+'.csv' #SPECIFY 
...
new_opt_pars, val_loss_per_epoch, training_losses, training_accs, train_sig_inputs, train_sig_outputs, val_sig_inputs, val_sig_outputs, epoch_c, save_w_sigs= two_layer_training.train_SSN_vmap(J_2x2_m, J_2x2_s, s_2x2_s, sigma_oris, c_E, c_I, w_sig, b_sig, ssn_pars, grid_pars, conn_pars_m, conn_pars_s, gE, gI, stimuli_pars, filter_pars, conv_pars, loss_pars, epochs_to_save, results_filename = results_filename, batch_size=batch_size, ref_ori = ref_ori, offset = offset, epochs=epochs, eta=eta, sig_noise = sig_noise, noise_type=noise_type, model_type=model_type, results_dir = results_dir)

if model_type !=4:
    epoch_c = None

#Plot losses
losses_dir = os.path.join(results_dir,'set_'+str(init_set)+'_model_type_'+str(model_type)+'_noise_type_'+str(noise_type)+'_both_grad_cont')
plot_losses(training_losses, val_loss_per_epoch, epochs_to_save[:len(val_loss_per_epoch)], epoch_c = epoch_c, save = losses_dir)

#Plot results
results_plot_dir = os.path.join(results_dir,'set_'+str(init_set)+'_model_type_'+str(model_type)+'_noise_type_'+str(noise_type)+'results_both_grad_cont')
plot_results_two_layers(results_filename, bernoulli = False, epoch_c = epoch_c, save= results_plot_dir)

#Plot sigmoid
sig_dir = os.path.join(results_dir,'set_'+str(init_set)+'_model_type_'+str(model_type)+'_noise_type_'+str(noise_type)+'_sigmoid_both_grad_cont')
...
#Plot training_accs
training_accs_dir = os.path.join(results_dir,'set_'+str(init_set)+'_model_type_'+str(model_type)+'_noise_type_'+str(noise_type)+'_training_accs_both_grad_cont') 
plot_training_accs(training_accs, epoch_c = epoch_c, save = training_accs_dir)

histogram_dir =os.path.join(results_dir,'set_'+str(init_set)+'_model_type_'+str(model_type)+'_noise_type_'+str(noise_type)+'_accuracy_histogram_both_grad_cont')
...
two_layer_training.test_accuracy(stimuli_pars, offset, ref_ori, J_2x2_m, J_2x2_s, s_2x2_s, c_E, c_I, w_sig, b_sig, sigma_oris, ssn_pars, grid_pars, conn_pars_m, conn_pars_s, gE, gI, filter_pars, conv_pars, loss_pars, sig_noise, noise_type, save = histogram_dir)
