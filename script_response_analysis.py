...
from SSN_classes_jax_jit import SSN2DTopoV1_ONOFF_local
from two_layer_training_lateral import evaluate_model_response, response_matrix, find_bins, bin_response,vmap_evaluate_response
...
epochs_to_analyse = np.asarray([0, 1198]).astype(int)
oris_to_analyse = np.asarray([55])#, 125, 0])
radius_list = np.asarray([3.0])


#Results to READ filename
results_filename = '/mnt/d/ABG_Projects_Backup/ssn_modelling/ssn-simulator/results/04-09/lateral/set_C_sig_noise_2.0_batch50_lamw1opt_1_results.csv'
...
saving_dir = os.path.join(os.getcwd(), 'results', '04-09', 'test_EIinput', '100_stimuli')
...
for epoch in epochs_to_analyse:
    ...
    J_2x2_m, J_2x2_s, c_E, c_I, f_E, f_I, sigma_oris, kappa_pre, kappa_post = load_param_from_csv(results_filename, epoch = epoch)
